...
// 제목에 [수시]/[정시] 같은 대괄호가 없는 일정(대학별 행사안내·박람회)은 code로 카테고리를 유추
const CODE_CATEGORY_MAP = { PGM: '행사', EXPO: '박람회' };
...
function parseEvents(html) {
  ...
  while ((m = re.exec(html))) {
    const [, code, , rawTitle, startDate, endDate, rawNote] = m;
    const title = decodeJsUnicode(decodeEntities(rawTitle));
    const categoryMatch = title.match(/^\[([^\]]+)\]/);
    const category = categoryMatch ? categoryMatch[1] : (CODE_CATEGORY_MAP[code] || '기타');
